...
from blog.forms.admin import EditProfileAdminForm
from blog.models import Role, User, Tag, Article, Comment
...
def index():
    ...
    reported_articles_count = Article.query.filter(Article.flag > 0).count()
    ...
    return render_template('admin/index.html', user_count=user_count, article_count=article_count,
                           ...
                           reported_articles_count=reported_articles_count)
...
def manage_user():
    ...
    per_page = current_app.config['BLOG_MANAGE_USER_PER_PAGE']
...
def manage_photo(order):
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['BLOG_MANAGE_PHOTO_PER_PAGE']
    order_rule = 'flag'
    if order == 'by_time':
...
def manage_tag():
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['BLOG_MANAGE_TAG_PER_PAGE']
