...
from blog.models import User, Article, Notification
from blog.notifications import push_collect_notification, push_follow_notification
...
@ajax_bp.route('/<int:article_id>/followers-count')
def collectors_count(article_id):
    article = Article.query.get_or_404(article_id)
    count = len(article.collectors)
    return jsonify(count=count)


@ajax_bp.route('/collect/<int:article_id>', methods=['POST'])
...
@ajax_bp.route('/uncollect/<int:article_id>', methods=['POST'])
